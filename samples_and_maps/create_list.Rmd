...
Now let's change sex by a name that we will replace in the RTF with symbols. Unfortunately, word can't read UTF8 encoding properly, and rtf package can't deal with other encodings.
```{r}
specimens = specimens %>%
  mutate(sex = ifelse(sex == 'm','MACHO',sex),
         sex = ifelse(sex == 'f','FEMEA',sex),
...
We will also use BBedit to change encoding from UTF8 to Mac OS Roman.
In word, we have to use find and replace to replace "MACHO" with ♂ and "FEMALE" with ♀, and double check any oddity in the species lists.
